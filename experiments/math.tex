...
&= -\expect_{\guide(\var)} [ \log \target(\var) - \log \guide(\var) ] + \log Z \\
...
&= \int \guide(\var) \log \frac{\unnormTarget(\var)}{\guide(\var)}d\var - \log Z \\
&= \expect_{\target(\var)} [ \log \target(\var) - \log \guide(\var) ] - \log Z \\
