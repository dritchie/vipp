...
% Importance-weighted ELBO gradient
&\grad \expect_{\guide(\var)} [ \log \frac{1}{k} \sum_{i=1}^{k} \frac{\target(\var_i)}{\guide(\var_i)} ] \\
&= \grad \expect_{\guide(\var)} [ \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) ] \\
&= \int \grad (\guide(\var)  \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) ) \\
&= \int \grad \guide(\var) \cdot \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) +  \int \guide(\var) \cdot \grad \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) \\
&= \int \guide(\var) \grad \log \guide(\var) \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) +  \int \guide(\var) \sum_{i=1}^{k} \normweight(\var_i) \grad \log \weight(\var_i) \\
&= \expect_{\guide(\var)} [ \grad \log \guide(\var) \log \frac{1}{k} \sum_{i=1}^{k} \weight(\var_i) - \sum_{i=1}^{k} \normweight(\var_i) \grad \log \guide(\var_i) ]