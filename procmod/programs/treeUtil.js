...
var makePivot = function() {
	var m1 = new THREE.Matrix4();
	var m2 = new THREE.Matrix4();
	return function (m, axis, angle, center) {
...
var inverseLerp = function() {
	var d = new THREE.Vector3();
	var tmp = new THREE.Vector3();
	return function (p, p0, p1) {
		d.copy(p1).sub(p0);
		var dnorm = d.lengthSq();
		return tmp.copy(p).sub(p0).dot(d) / dnorm;
	};
}();

var projectOnLineSeg = function() {
	var d = new THREE.Vector3();
	var tmp = new THREE.Vector3();
	return function (p, p0, p1) {
		d.copy(p1).sub(p0).normalize();
		var s = tmp.copy(p).sub(p0).dot(d);
		d.multiplyScalar(s);
		p.copy(d).add(p0);
		return p;	
	};
...
var circleOfVerts = function() {
	...
	return function (geo, frame) {
		...
		makePivot(m, frame.forward, rotamt, frame.center);
...
var weldCircleOfVerts = function() {
	...
	return function (geo, frame, prev) {
		...
		for (var i = 0; i < N_SEGS + 1; i++) {
			...
			projectOnLineSeg(p.copy(v), prev.p0, prev.p1);
			var t = inverseLerp(p, prev.p0, prev.p1);
...
var branchFrame = function() {
	...
	return function (startFrame, endFrame, t, theta, radius) {
		...
		makePivot(m, endFrame.forward, theta, ct);
		makePivot(m1, endFrame.forward, theta, endFrame.center);
		...
		var lopoint = projectOnLineSeg(upbf.clone().multiplyScalar(radius).negate().add(cbf), startFrame.center, endFrame.center);
		var lot = inverseLerp(lopoint, startFrame.center, endFrame.center);
		var hipoint = projectOnLineSeg(upbf.clone().multiplyScalar(radius).add(cbf), startFrame.center, endFrame.center);
		var hit = inverseLerp(hipoint, startFrame.center, endFrame.center);
